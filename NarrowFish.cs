...
public class NarrowFish : MonoBehaviour
{
    ...
    {
        narrowFish = FindObjectsOfType<NarrowFish>();

        foreach (var i in narrowFish)
        {
            if (i.selected)
            {
                ...
                selected = false;
                Debug.Log("Destroyed");
                int temp = PlayerPrefs.GetInt("nNarrowFish");
                temp--;
                PlayerPrefs.SetInt("nNarrowFish", temp);
