...
public class OriginalFish : MonoBehaviour
{
    ...
    {
        originalFish = FindObjectsOfType<OriginalFish>();

        foreach (var i in originalFish)
        {
            if (i.selected)
            {
                ...
                int temp = PlayerPrefs.GetInt("nOriginalFish");
                temp--;
                PlayerPrefs.SetInt("nOriginalFish", temp);
