...
public class Seahorse : MonoBehaviour
{
    ...
    IEnumerator SwitchDirection()
    {
        ...
        if ((rb2D.position.x > 6 && direction.x > 0) || (rb2D.position.x < -6 && direction.x < 0))
        {
            switchTime = 7;
    ...
    {
        seahorses = FindObjectsOfType<Seahorse>();

        foreach (var i in seahorses)
        {
            if (i.selected)
            {
                ...
                {
                    int temp3 = PlayerPrefs.GetInt("nSeahorse");
                    temp3--;
                    PlayerPrefs.SetInt("nSeahorse", temp3);
